...
public class test {
	...
	public static void main(String[] args) throws IOException {
        ...
        java.lang.System.out.printf("image type : %d ", image.getType());
		...
		for (int i = 0; i < 5; i++) {
			...
			for (int j = 0; j < 3*5*1920; ) {
				img.getColorModel().getComponents(img.getData().getDataBuffer().getElem(j), pixel,0);
				img.getData().getDataBuffer().setElem(j, 255);
				j++;
				img.getData().getDataBuffer().setElem(j, pixel[1]);
				j++;
				img.getData().getDataBuffer().setElem(j, pixel[2]);
				j++;
			}
