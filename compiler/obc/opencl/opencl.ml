...
and cstm =
  ...
  | Cfor of string * cexpr * cexpr * cexpr * cstm list (** For loop. int <= string < int by int *)
...
and pp_cstm fmt stm = match stm with
  ...
  | Cfor(x, lower, upper, step, e) when step = Cconst (Ccint 1)->
      fprintf fmt "@[<v>@[<v 2>for (int %a = %a; %a < %a; ++%a) {%a@]@ }@]"
        pp_string x  pp_cexpr lower  pp_string x
        pp_cexpr upper  pp_string x  pp_cstm_list e
  | Cfor(x, lower, upper, step, e)->
      fprintf fmt "@[<v>@[<v 2>for (int %a = %a; %a < %a; %a += %a) {%a@]@ }@]"
        pp_string x  pp_cexpr lower  pp_string x  pp_cexpr upper
        pp_string x  pp_cexpr step  pp_cstm_list e
  | Cwhile (e, b) ->
