...
let print_partition_idx fmt (p:int) =
  Format.fprintf fmt "Partition:%d" p

let print_block_idx fmt b =
  Format.fprintf fmt "Block:%d" b.block_index

let print_const_exp fmt ce =
  match ce with 
...
let print_fun_def fmt fun_def =
  let print_arg fmt (arg,at_opt) =
    let print_arg_no_at fmt arg =
      Format.fprintf fmt "%s:%a"
        arg.arg_name
        print_ty_idx arg.arg_type
    in
    match at_opt with
    | None -> print_arg_no_at fmt arg
    | Some name ->
  ...
  Format.fprintf fmt "%a\t%a\t(%a) -> (%a)\n"
    ...
    (* Input arguments have no AT annotation in GC *)
    (print_list print_arg) (List.map (fun (x,y) -> (x,None)) fun_def.fun_inputs)
...
let print_const_def fmt cst_def =
  ...
  begin
    match cst_def.cst_desc with
    | ExternalConst ->
       Format.fprintf fmt "External"
    | LiteralConst lit ->
       Format.fprintf fmt "%a" print_const_exp lit
...
let print_var_def fmt var_def =
  ...
  Format.fprintf fmt "%a\t%s\t%a\tSingle Assignment\t%a @@ %a"
                 print_var_idx var_def
                 (match var_def.var_name with | None -> "" | Some s -> s)
                 print_ty_idx var_def.var_type
                 print_id source_id
                 print_block_idx source_block ;
  begin
    match var_def.var_allocation with
    | Some _ ->
       (* Format.fprintf fmt " On %a\n" ;*) raise Not_implemented
    | None -> ()
  end;
  begin
    match var_def.var_at with
    | None -> ()
    | Some idx -> Format.fprintf fmt " @@ Variable:%d" idx
  end;
  Format.fprintf fmt "\n" 
...
let print_clk_def fmt clock_def =
  ...
  Format.fprintf fmt "\t" ;
  begin
    match clock_def.clk_desc with
    | Never -> Format.fprintf fmt "TODO" 
    | Primitive -> Format.fprintf fmt "Primitive"
    | Derived e ->
       print_clk_exp fmt e
  end ;
  Format.fprintf fmt "\t%a\n"
    (print_list print_clk_rval) clock_def.clk_dependencies
...
let print_partition_def fmt part =
  Format.fprintf fmt "%a\t%a\n"
    print_partition_idx part.part_index
...
let print_block_def fmt block =
  let print_input_port fmt ip =
    Format.fprintf fmt "%a Is %a"
      ...
      (print_list print_clk_rval) ip.input_port_arcs
  ...
  begin
    match block.block_offset with
    | Some i -> Format.fprintf fmt "\tOffset:%d" i
    | None -> ()
  end ;
  begin
    match block.block_deadline with
    | Some i -> Format.fprintf fmt "\tDeadline:%d" i
    | None -> ()
  end ;
  begin
    match block.block_partition with
    | None -> ()
    | Some p -> Format.fprintf fmt "\tBelongs To:%a" print_partition_idx p
  end ;
...
let print_clocked_graph (fmt : Format.formatter) (cg : clocked_graph) =
  ...
  match cg.period with
  | None -> () ;
  | Some p ->
     pretty_print_head1 fmt ("MTF: "^(string_of_int p)^"\n") 
