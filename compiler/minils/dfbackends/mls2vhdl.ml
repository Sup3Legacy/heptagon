...
let trans_ty bty = Vt_id { qual = "Pervasives"; name = mk_name_bty bty; }
...
let trans_ty_dec tyd =
  let desc = match tyd.t_desc with
    ...
    | Type_struct nbtyl ->
        let mk_field { f_name = n; f_type = ty; } =
          (shortname n, trans_ty ty) in
        Vty_record (List.map mk_field nbtyl)
    | Type_abs -> Vty_opaque
    | Type_alias _ -> failwith "unimplemented" in
...
let tb_node nd =
  ...
  let tb_name = bench_name (shortname nd.n_name) in

  (** [ci_name] will be the name of our instantiated component. *)
  let ci_name = shortname nd.n_name ^ "_0"
  ...
  and ent_name = { nd.n_name with qual = "work"; } in
  ...
  let decls =
    ...
    Vd_component (shortname nd.n_name, in_signals @ out_signals)
    :: Vd_bind (ci_name, shortname nd.n_name, ent_name)
  ...
  let comp_inst =
    ...
    Vdef_comp_inst (ci_name, shortname nd.n_name, bindl) in
...
let translate modn p =
  ...
  (match !Compiler_options.simulation_node with
     ...
     | Some qn ->
         let nd_to_simulate =
           try List.find (fun nd -> nd.n_name = qn) p.p_nodes
           with Not_found ->
             Format.eprintf "Unknown node to simulate: %a@." print_qualname qn;
             assert false in
